...
import { physics9Pages, PhysicsPage } from '../data/physics9Data';

interface Physics9ViewerProps {
  onClose: () => void;
}

export default function Physics9Viewer({ onClose }: Physics9ViewerProps) {
  ...
  const pageObj = useMemo(() => {
    return physics9Pages.find(p => p.pageNumber === currentPage) || physics9Pages[0];
  }, [currentPage]);
    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-slate-900 text-slate-100 font-sans print:bg-white print:text-black">
      ...
      <header className="flex h-16 shrink-0 items-center justify-between border-b border-slate-800 bg-slate-950 px-6 print:hidden">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-heading text-base font-bold text-white flex items-center gap-1.5 leading-none">
              Class 9th Physics Cheat Sheet
              <span className="rounded-full bg-blue-500/15 px-2 py-0.5 text-[10px] font-extrabold text-blue-400 border border-blue-500/30">
                OFFICIAL ACCESS
              </span>
            </h1>
            <p className="text-xs text-slate-400 font-medium mt-1">Interactive Study Kit & Kinematics Formulation Guide</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          <button
            onClick={handlePrint}
            className="flex items-center gap-1.5 rounded-full bg-blue-600 hover:bg-blue-500 px-4.5 py-2 font-heading text-xs font-bold text-white shadow-lg shadow-blue-500/15 transition-all hover:scale-105 active:scale-95 cursor-pointer"
          >
          ...
          <div className="p-4 border-b border-slate-800">
            <div className="relative">
              <input
                ...
                className="w-full bg-slate-900 border border-slate-700 rounded-xl py-2 pl-9 pr-4 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
              />
              ...
              <div className="space-y-2">
                ...
                {searchResults.length > 0 ? (
                  searchResults.map(p => (
                    ...
                    >
                      <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded bg-blue-500/10 text-blue-400 text-[10px] font-bold group-hover:bg-blue-500 group-hover:text-white transition-colors">
                        {p.pageNumber}
                      </span>
                        ...
                        {ch.pages.map(p => (
                          <button
                            ...
                            className={`w-full text-left py-1 px-2 rounded text-[11px] font-medium transition-all block truncate ${
                              currentPage === p.num
                                ? 'bg-blue-500/10 text-blue-400 border-l-2 border-blue-500 pl-1.5 font-bold'
        ...
        <main className="flex-1 bg-slate-900 overflow-y-auto p-6 md:p-10 flex flex-col items-center justify-between print:bg-white print:p-0 print:block print:overflow-visible">
          ...
          <div className="w-full max-w-3xl flex justify-between items-center mb-4 text-xs text-slate-400 print:hidden">
            <span className="flex items-center gap-1.5">
              ...
              Viewing page <strong className="text-white">{currentPage}</strong> of 20
            </span>
          ...
          <div className="w-full max-w-3xl bg-slate-950 rounded-2xl shadow-2xl p-8 md:p-12 border border-slate-800 min-h-[640px] flex flex-col justify-between relative print:bg-white print:text-black print:border-none print:shadow-none print:p-0 print:min-h-0 print:block">
            ...
            <div className="flex items-center justify-between border-b border-slate-800/80 pb-4 print:border-slate-200">
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-extrabold text-blue-400 tracking-wider uppercase font-mono">
            ...
            <div className="flex-1 my-2 print:my-4">
                  ...
                  <div className="grid grid-cols-1 gap-4.5">
                    {pageObj.content.concepts.map((c: any, idx: number) => (
                      <div key={idx} className="p-4 rounded-xl border border-slate-800/80 bg-slate-900/40 print:bg-slate-50 print:border-slate-100">
                        <h4 className="font-heading text-sm font-extrabold text-blue-400 flex items-center gap-2 print:text-blue-700">
                          <span className="flex h-5 w-5 items-center justify-center rounded bg-blue-500/10 text-blue-400 text-[10px] font-extrabold">
                            {idx + 1}
                          </span>
                          {c.heading}
                        </h4>
              ...
              {/* Type 2: Table / Formula & Fact Bank */}
              {pageObj.type === 'table' && (
                <div className="space-y-4">
                      ...
                      <tbody className="divide-y divide-slate-800/60 print:divide-slate-200 text-slate-300 print:text-black">
                        {pageObj.content.rows.map((row: any, idx: number) => (
                          <tr key={idx} className="hover:bg-slate-900/20">
                            ...
                            <td className="px-4 py-3.5 font-mono text-blue-300 print:text-blue-900 font-bold">{row.plant}</td>
                      ...
                      return (
                        <div key={prob.num} className="border border-slate-800 rounded-xl overflow-hidden bg-slate-900/10 print:border-slate-200">
                          ...
                          <div className="bg-slate-900/60 px-4 py-3 border-b border-slate-850 flex items-center justify-between print:bg-slate-50">
                            <div className="flex items-center gap-2">
                              <span className="text-xs font-bold text-blue-400 print:text-blue-700">Problem {prob.num}</span>
                              <span className="h-1.5 w-1.5 rounded-full bg-slate-700" />
                              <span className="rounded bg-slate-800 px-2 py-0.5 text-[9px] font-bold text-slate-400 uppercase tracking-wider">{prob.type}</span>
                            </div>

                            <button
                              onClick={() => toggleSolution(uid)}
                              className="inline-flex items-center gap-1 text-[10px] font-bold text-blue-400 hover:text-blue-300 cursor-pointer print:hidden"
                            >
                              {isOpen ? (
                                <>
              ...
              {Array.from({ length: 20 }, (_, i) => i + 1).map(num => (
                <button
                  ...
                  className={`h-2 rounded-full transition-all cursor-pointer ${
                    currentPage === num ? 'w-5 bg-blue-500' : 'w-2 bg-slate-700 hover:bg-slate-600'
                  }`}
